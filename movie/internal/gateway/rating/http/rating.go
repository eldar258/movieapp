...
func (g *Gateway) GetAggregatingRating(ctx context.Context, recordId model.RecordID, recordType model.RecordType) (float64, error) {
	...
	if resp.StatusCode == http.StatusNotFound {
		...
		return 0, fmt.Errorf("non-2xx responce: %v", resp)
	}
...
func (g *Gateway) PutRating(ctx context.Context, recordId model.RecordID, recordType model.RecordType, rating model.Rating) error {
	req, err := http.NewRequest(http.MethodPost, g.addr+"/rating", nil)
	if err != nil {
		return err
	}
	...
	if resp.StatusCode/2 != 2 {
		return fmt.Errorf("non-2xx responce: %v", resp)
	}
